...
- **Don't modify `ARCHITECTURE.md` decisions without flagging it.** If
  ...
  This explicitly includes: the no-Docker-socket constraint (don't add
  Docker socket access to "simplify" the setup wizard without flagging it
  first), and the distinction between automatic RCA (scheduled polling)
  and real anomaly detection (still roadmap/disabled) — don't blur that
  line in code, comments, or UI copy.
