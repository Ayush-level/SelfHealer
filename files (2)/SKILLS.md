...
| React (SPA, API-driven) | `frontend/`, Phase 7 | No server-rendered pages anywhere — the frontend only ever talks to the Flask JSON API. See `FRONTEND.md`. |
| SigNoz operation (its own OTel Collector + ClickHouse schema) | `docker-compose.signoz.yml`, `config/signoz-otel-collector-config.yaml`, Phase 8 | SigNoz's collector is a second, dedicated collector receiving a forwarded copy — it is not the same collector instance as `otel-collector`. Don't merge them. |
| APScheduler (interval-based background jobs) | `proxy/scheduler/rca_scheduler.py`, Phase 9 | This is scheduled polling, not anomaly detection — see `ARCHITECTURE.md`'s RCA Trigger Modes scope note before implying otherwise in code comments or the demo narration. |
