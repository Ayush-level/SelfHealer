...
- [x] **3.3** Make Prometheus's own published port conditional on an
...
- [x] **7.1** Scaffold the React SPA (`frontend/`) with routing for
...
- [x] **7.2** Build `proxy/routes/config.py`: `GET/POST /api/config`, writing
...
- [x] **7.3** Build the Setup wizard page: on save, display the exact
...
- [x] **7.4** Build `proxy/routes/tools.py` (`GET /api/tools`) returning
  enabled tool URLs from saved config.
  **Test:** toggling a tool off in config removes it from the `/api/tools` response.
- [x] **7.5** Build the Home page against `GET /api/health` and the
